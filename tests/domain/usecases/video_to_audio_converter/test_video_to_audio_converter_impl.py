...
class TestVideoToAudioConverterUseCase:
    ...
    @pytest.mark.asyncio
    async def test_should_return_an_AudioMedia_when_video_converter_is_success(
        self, video_to_audio_usecase, video_converter_service_stub
    ):
    ...
    @pytest.mark.asyncio
    async def test_should_return_an_response_with_Exception_when_video_converter_has_failed(
        self, video_to_audio_usecase, video_converter_service_stub
    ):
        video = VideoSource(title="Real Estate - Paper Cup", thumbnail_url="any_thumbnail_url", path="any_path")
